        ...
        for path in paths:
            with path.open() as f:
                for line in f:
                    ...
                    if len(fields) == 2:
                        pairs.setdefault(int(fields[0]), set()).add(int(fields[1]))
                        pairs.setdefault(int(fields[1]), set()).add(int(fields[0]))
                    elif len(fields) == 3:
                        s_id, s_lang, s_body = fields
                        if s_lang == self.target_lang:
                            self.sentences[int(s_id)] = {'body': s_body.strip()}
                        else:
                            sources[int(s_id)] = s_body.strip()
                    else:
                        pass
        for s_id, s_obj in self.sentences.items():
            s_obj['words'] = {word.casefold() for word in re.findall(r'\w+', s_obj['body'])}
            s_obj['translations'] = {sources[src_id] for src_id in pairs.get(s_id, [])}
            for word in s_obj['words']:
                self.words.setdefault(word, set()).add(s_id)
        return len(self.sentences)

    def most_common_words(self, limit=None):
        return Counter(word for s_obj in self.sentences.values() for word in s_obj['words']).most_common(limit)
...
def learn():
    ...
    while True:
        ...
        def difficulty(s_id):
            ...
            return (len(new_words), sum(popularity[word] for word in new_words)) if new_words else (999, 999999)
